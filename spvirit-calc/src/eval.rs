    ...
    pub fn eval(&self, args: &[f64; 21]) -> f64 {
        ...
        for op in &self.ops {
            match op {
                ...
                // Task 5. Base's COND_IF/COND_ELSE (calcPerform.c:400-410)
                // short-circuits by jumping over the untaken branch in the
                // instruction stream - it never evaluates both branches and
                // discards one. A naive "pop three, pick one" evaluator
                // here would be observably wrong for a CALC expression with
                // side-effect-free-but-diverging branches (e.g. one side
                // dividing by an operand the other side guards against) in
                // ways that don't matter for pure arithmetic but do for the
                // general grammar. Left unimplemented per RULINGS.md
                // guidance rather than inventing eager-evaluation
                // semantics now.
                Op::Cond => unimplemented!("Op::Cond is implemented in Task 5"),
            }
        }

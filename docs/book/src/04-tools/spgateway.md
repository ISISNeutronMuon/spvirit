...
| RPC | `threads` | RPC-only `NTScalar('s')` (string), matching p4p's `threads` PV, which is served as an RPC that dumps a thread traceback. spvirit returns a shape-correct best-effort string (available-parallelism plus a note that an OS-level stack dump is not collected and that work runs on tokio async tasks). Not offered as a get/subscribe value PV. |

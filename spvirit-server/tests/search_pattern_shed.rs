...
#[tokio::test]
async fn shedding_increments_a_counter() {
    let name_calls = Arc::new(AtomicUsize::new(0));
    let addr = spawn_tcp(name_calls.clone()).await;
    let mut stream = handshake(addr).await;
    saturate(&mut stream, &name_calls).await;

    // Process-wide and monotonic, so this asserts on a delta the test itself
    // causes; a neighbour shedding concurrently can only make it larger.
    let before = global_stats().pattern_enum_shed;
    const EXTRA: u32 = 3;
    for i in 0..EXTRA {
        let req = encode_search_request(
    ...
    // Nothing replies, so wait for the counter rather than for a frame.
    for _ in 0..100 {
        if global_stats().pattern_enum_shed >= before + u64::from(EXTRA) {
            return;
        }
        tokio::time::sleep(Duration::from_millis(20)).await;
    }
    panic!(
        "{EXTRA} pattern queries were shed but `pattern_enum_shed` only moved \
         from {before} to {}; shedding is invisible to /metrics",
